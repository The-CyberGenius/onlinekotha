            ...
            if (mobileSearchResults) {
                if (filteredMsgs.length === 0) {
                    ...
                    limitRes.forEach(msg => {
                        const highlightedText = (msg.text || '').replace(regex, `<mark class="bg-yellow-200 text-gray-900 font-bold px-0.5 rounded">$1</mark>`);
                        resultsHtml += `
                            <div class="p-2 bg-gray-50 dark:bg-[#1a2329] hover:bg-indigo-50 dark:hover:bg-gray-700/50 shadow-sm cursor-pointer border border-gray-100 dark:border-gray-800 transition-all rounded-lg mb-1.5" onclick="jumpToMsg(${msg.id}); closeMobileOverlay();">
                                <div class="flex justify-between items-center mb-0.5">
                                    <span class="text-[10px] font-bold uppercase tracking-wide" style="color:${typeof getStringColor !== 'undefined' ? getStringColor(msg.sender || 'System') : '#666'}">${msg.sender || 'System'}</span> 
                                    <span class="text-[9px] text-gray-400 font-semibold">${msg.date || ''} ${msg.time || ''}</span>
                                </div>
                                <p class="text-[11px] text-gray-700 dark:text-gray-200 font-medium line-clamp-2 leading-relaxed">${highlightedText}</p>
                            </div>
